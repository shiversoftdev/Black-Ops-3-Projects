...
#region ResumeThread
        ...
        #region NtSuspendProcess
#if USE_PINVOKE
        [DllImport("ntdll.dll", SetLastError = true)]
        private static extern void NtSuspendProcess(PointerEx hProc);
        public static void NtSuspendProcessP(PointerEx hProc)
        {
            return NtSuspendProcess(hProc);
        }
#else
        public static void NtSuspendProcessD(PointerEx hProc)
        {
            object[] funcArgs = new object[] { hProc };
            Evasion.DInvoke.DynamicAPIInvoke(CONST_NTDLL, "NtSuspendProcess", typeof(_NtSuspendProcess), ref funcArgs);
        }
#endif
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate void _NtSuspendProcess(PointerEx hProc);

        public static void NtSuspendProcessM(PointerEx hProc)
        {
            object[] funcArgs = new object[] { hProc };
            Evasion.DInvoke.ManualInvoke(CONST_NTDLL, "NtSuspendProcess", typeof(_NtSuspendProcess), ref funcArgs);
        }
        #endregion

        #region NtResumeProcess
#if USE_PINVOKE
        [DllImport("ntdll.dll", SetLastError = true)]
        private static extern void NtResumeProcess(PointerEx hProc);
        public static void NtResumeProcessP(PointerEx hProc)
        {
            return NtResumeProcess(hProc);
        }
#else
        public static void NtResumeProcessD(PointerEx hProc)
        {
            object[] funcArgs = new object[] { hProc };
            Evasion.DInvoke.DynamicAPIInvoke(CONST_NTDLL, "NtResumeProcess", typeof(_NtResumeProcess), ref funcArgs);
        }
#endif
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        public delegate void _NtResumeProcess(PointerEx hProc);

        public static void NtResumeProcessM(PointerEx hProc)
        {
            object[] funcArgs = new object[] { hProc };
            Evasion.DInvoke.ManualInvoke(CONST_NTDLL, "NtResumeProcess", typeof(_NtResumeProcess), ref funcArgs);
        }
        #endregion
    ...
    public static class NativeStealth
    {
        ...
        public static Native._NtSuspendProcess NtSuspendProcess { get; private set; }
        public static Native._NtResumeProcess NtResumeProcess { get; private set; }
        ...
        private static void StealthUpdate()
        {
            if(Stealth == NativeStealthType.ManualInvoke)
            {
                ...
                NtSuspendProcess = Native.NtSuspendProcessM;
                NtResumeProcess = Native.NtResumeProcessM;
            }
            else
            {
#if USE_PINVOKE
                ...
                NtSuspendProcess = Native.NtSuspendProcessP;
                NtResumeProcess = Native.NtResumeProcessP;
#else
                ...
                NtSuspendProcess = Native.NtSuspendProcessD;
                NtResumeProcess = Native.NtResumeProcessD;
#endif
            }
        }
