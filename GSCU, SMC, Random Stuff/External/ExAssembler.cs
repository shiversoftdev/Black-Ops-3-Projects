...
namespace System
{
    public static class ExAssembler
    {
        ...
        private static byte[] CreateRemoteCall64(PointerEx jumpLocation, PointerEx[] args, PointerEx raxStorAddress, PointerEx threadStateAddress, byte xmmMask, ExXMMReturnType xmmReturnType)
        {
            ...
            int savedPos = data.Count; // save this location so we can insert shellcode to align stack to 0x10 boundary if needed
            int stackAdjust = 0;
            ...
            if((stackAdjust % 0x10) > 0)
            {
                data.InsertRange(savedPos, new byte[] { 0x48, 0x83, 0xec, 0x08 }); // align stack correctly so SIMD ops dont fail
            }
