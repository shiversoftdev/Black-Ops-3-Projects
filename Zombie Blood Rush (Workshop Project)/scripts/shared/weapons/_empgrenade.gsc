...
function applyemp(attacker, explosionpoint)
{
    ...
    ratio = max(0, 1 - (distancetoexplosion / 300));
    currentempduration = 5 + (3 * ratio);
    ...
    if(isfunctionptr(level.zbr_emp_cb))
    {
        self [[ level.zbr_emp_cb ]](true);
    }
...
function watchempexplosion(owner, weapon)
{
	...
	level empexplosiondamageents(owner, weapon, origin, 300, 1);
}
...
{
    if(isfunctionptr(level.zbr_emp_explode))
    {
        level thread [[ level.zbr_emp_explode ]](owner, weapon, origin, radius, damageplayers);
    }
    level notify("emp_detonate", origin, radius);
...
function begin_other_grenade_tracking()
{
	...
	for(;;)
	{
		...
		if(weapon.isemp === true)
		{
			if(isdefined(level.zbr_grenade_emp_watcher))
			{
				grenade thread [[ level.zbr_grenade_emp_watcher ]](self, weapon);
			}
